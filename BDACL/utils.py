...
##### check whether input is suitable for BDACL preprocessing #####
def checkInput(adata,batch_key,log):
    ...
    if batch_key is not None and batch_key not in adata.obs.columns:
        ...
        log.info("BDACL cretate \"BATCH\" column to set all cell to one batch!!!")
        batch_key="BATCH"
...
#######plot HeatMap with similarity calculated by BDACL ######
...
def plotSankey(df, cat_cols=[], value_cols='', title='Sankey Diagram'):
    ...
    data = dict(type='sankey',
                ...
                # node=dict(pad=10, thickness=20, line=dict(color="black", width=0.5), label=labelList),
                link=dict(source=sourceTargetDf['sourceID'], target=sourceTargetDf['targetID'],
                          value=sourceTargetDf['count'], color="darkgray"),

                )
    layout = dict( title=dict(text=title, font=dict(size=20, color="black")), font=dict(size=10,color="black"), paper_bgcolor='white')
